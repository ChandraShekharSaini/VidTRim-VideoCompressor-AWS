...
app.use(
  cors({
    origin: [process.env.FRONTEND_PUBLIC_IP, "http://localhost:5173", "http://35.169.44.254","http://spring-load-balancer-37703404.us-east-1.elb.amazonaws.com"],
    credentials: true,
...
app.get(
  ...
  GoogleAuthPassport.authenticate("google", {
    failureRedirect: `http://${process.env.FRONTEND_PUBLIC_IP}/account-create/sign-in`,
    session: false,
  }),
  function (req, res) {
    if (!req.user)
      return res.redirect(
        `http://${process.env.FRONTEND_PUBLIC_IP}/account-create/sign-in`,
      );
    ...
    res.redirect(
      `http://${process.env.FRONTEND_PUBLIC_IP}?token=${encodeURIComponent(
